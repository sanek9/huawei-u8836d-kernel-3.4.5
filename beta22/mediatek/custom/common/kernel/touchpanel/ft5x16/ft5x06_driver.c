...
int tp_charger_detect(int current_charge_status)
{
       ...
       int ret =-1;
	static char pre_charge_status=0;
	ret = fts_read_reg(0x8b,&pre_charge_status);
	if (ret < 0) {
		printk("read 0x8b  failed");
	       return ret;
	}
...
static  void tpd_down(tinno_ts_data *ts, int x, int y, int pressure, int trackID) 
{
	...
	tp_charger_detect(g_charger_in_flag);
 }
...
#if defined(TINNO_ANDROID_S9096)
{
	...
	sprintf(tpd_desc, "%s-S9096-%s-%d", fts_get_vendor_name(panel_vendor),
