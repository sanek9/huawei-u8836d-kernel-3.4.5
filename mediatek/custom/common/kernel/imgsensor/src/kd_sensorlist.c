	...
	else
	{
		...
		strcpy(&g_currSensorNames[0][0],g_currSensorName);
	     ...
	     else
	     {
	       ...
	       if((sensorID==OV8825_SENSOR_ID)||(sensorID==OV8826_SENSOR_ID)||(sensorID==OV8827_SENSOR_ID))
	       {
			extern volatile int  camera_sensor_type;

			if(camera_sensor_type==1)
			{
				strcpy(&g_currSensorNames[1][0],"ov8825-truly-new");
			}
			else if(camera_sensor_type==2)
			{
				strcpy(&g_currSensorNames[1][0],"ov8825-shunyu-old");
			}
			else if(camera_sensor_type==3)
			{
				strcpy(&g_currSensorNames[1][0],"ov8825-shunyu-MM");
			}
			else if(camera_sensor_type==4)
			{
				strcpy(&g_currSensorNames[1][0],"ov8825-truly-old");
			}
			else if(camera_sensor_type==5)
			{
				strcpy(&g_currSensorNames[1][0],"ov8825-truly-MM");
			}
		}
		else
	     #endif
...
#endif 
{
    ...
    PK_DBG("[CAMERA SENSOR] CAMERA_HW_Ioctl: %x, %x \n",a_u4Command,a_u4Param);
    mutex_lock(&kdCam_Mutex); 
